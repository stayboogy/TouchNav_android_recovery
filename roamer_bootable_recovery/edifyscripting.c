...
int run_and_remove_extendedcommand()
{
    ...
    ui_print("verifying sdcard marker...\n");
    struct stat st;
    if (stat("/sdcard/clockworkmod/.salted_hash", &st) != 0) {
        ui_print("sdcard marker not found...\n");
        if (volume_for_path("/emmc") != NULL) {
            ui_print("checking internal sdcard marker...\n");
            ensure_path_unmounted("/sdcard");
            if (ensure_path_mounted_at_mount_point("/emmc", "/sdcard") != 0) {
                ui_print("internal sdcard marker not found... continuing anyways.\n");
